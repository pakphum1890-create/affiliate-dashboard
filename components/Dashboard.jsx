...
async function loadOrSeedTable(table, seedRows, orderBy = "id") {
  ...
  if (data.length === 0 && seedRows.length > 0) {
    const rows = seedRows.map(({ id, ...rest }) => rest);
    const { data: inserted, error: insertError } = await supabase.from(table).insert(rows).select();
    if (insertError) {
      console.error(`Supabase seed error (${table}):`, insertError.message);
      return data;
    }
    return inserted;
  }
  ...
  useEffect(() => {
    ...
    (async () => {
      const [v, p, s, c] = await Promise.all([
        loadOrSeedTable("videos", initialVideos),
        loadOrSeedTable("products", initialProducts),
        loadOrSeedTable("sponsors", initialSponsors),
        loadOrSeedTable("calendar", initialCalendar, "date"),
      ]);
      if (v) setVideoList(v);
