...
const fontStyle = (
  <style>{`
    ...
    .row-del { opacity: 0.55; transition: opacity .2s ease; }
    .row-del:hover, .row-del:active { opacity: 1; }
    .cell-del { opacity: 0.7; transition: opacity .2s ease; }
    .cell-del:hover, .cell-del:active { opacity: 1; }
  `}</style>
);
...
/* ---------- Supabase helpers ----------
   If `supabase` isn't configured (no env vars), the dashboard silently
   stays in local-only mode: it still works, but nothing is saved
   between sessions. Once NEXT_PUBLIC_SUPABASE_URL / ANON_KEY are set
   on Vercel, every add/delete is written straight to your database.
   Note: this does NOT seed your tables with sample data — it only
   loads whatever real rows you've added yourself (which may be none
   yet, and that's fine, the UI shows an empty state). */
async function loadTable(table, orderBy = "id") {
...
export default function Dashboard() {
  ...
  // lifted, editable state — starts empty when Supabase is connected
  // (real data loads in the effect below); falls back to sample data
  // only when running without a database connection.
  const [videoList, setVideoList] = useState(supabase ? [] : initialVideos);
  const [productList, setProductList] = useState(supabase ? [] : initialProducts);
  const [sponsorList, setSponsorList] = useState(supabase ? [] : initialSponsors);
  const [calendarList, setCalendarList] = useState(supabase ? [] : initialCalendar);
  const [liveStats, setLiveStats] = useState(
    supabase ? { id: 1, sessions: 0, sales: 0, avg_viewers: 0, peak_viewers: 0 } : { id: 1, sessions: 14, sales: 9840, avg_viewers: 312, peak_viewers: 1204 }
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      const [v, p, s, c] = await Promise.all([
        ...
        loadTable("products"),
        loadTable("sponsors"),
        loadTable("calendar", "date"),
      ]);
      if (v) setVideoList(v);
      if (p) setProductList(p);
      if (s) setSponsorList(s);
      if (c) setCalendarList(c);

      const { data: liveRow, error: liveError } = await supabase.from("live_stats").select("*").eq("id", 1).maybeSingle();
      if (!liveError && liveRow) setLiveStats(liveRow);
    ...
    <div className="f-body min-h-screen flex" style={{ background: bg }}>
      ...
      <aside className="w-60 shrink-0 hidden md:flex flex-col justify-between sticky top-0 h-screen" style={{ background: T.ink }}>
        ...
        <div className="p-4 m-3 rounded-xl" style={{ background: "#1C1C24" }}>
          <p className="text-xs text-white f-body mb-1">ยอดขายรวมตอนนี้</p>
          <p className="text-lg f-display font-semibold text-white">
            ฿{productList.reduce((a, p) => a + (Number(p.sales) || 0), 0).toLocaleString()}
          </p>
          <p className="text-[11px] mt-1.5" style={{ color: T.muted }}>
            คำนวณจาก {productList.length} รายการสินค้าที่คุณเพิ่มไว้
          </p>
        </div>
      </aside>
      ...
      <div className="flex-1 min-w-0">
        ...
        >
          ...
          <span
            className="hidden lg:flex items-center gap-1.5 text-[11px] f-body px-2.5 py-1.5 rounded-full"
            style={{
              background: supabase ? "#16A34A14" : T.muted + "14",
              color: supabase ? "#16A34A" : T.muted,
            }}
          >
            <span className="w-1.5 h-1.5 rounded-full" style={{ background: supabase ? "#16A34A" : T.muted }} />
            {supabase ? (syncing ? "กำลังซิงค์..." : "เชื่อมฐานข้อมูลแล้ว") : "โหมดออฟไลน์"}
          </span>
          ...
          <div className="relative">
            ...
            {notifOpen && (
              <div className="absolute right-0 mt-2 w-64 bg-white rounded-xl border shadow-lg p-3 z-30 flex flex-col gap-3" style={{ borderColor: T.line }}>
                <NotifRow icon={Sparkles} color={T.violet} text="ยังไม่มีการแจ้งเตือนตอนนี้" time="ระบบจะแจ้งเมื่อมีความเคลื่อนไหวจริง" />
              </div>
            )}
          </div>
        ...
        <div className="p-5 max-w-7xl mx-auto">
          {tab === "overview" && (
            <OverviewTab textMain={textMain} border={border} videoList={videoList} productList={productList} calendarList={calendarList} />
          )}
          {tab === "calendar" && (
          ...
          {tab === "live" && <LiveTab liveStats={liveStats} setLiveStats={setLiveStats} />}
...
function EmptyState({ icon: Icon, text, hint }) {
  return (
    <div className="flex flex-col items-center justify-center py-10 text-center gap-2">
      <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: T.paper }}>
        <Icon size={17} color={T.muted} />
      </div>
      <p className="text-xs f-body" style={{ color: T.muted }}>{text}</p>
      {hint && <p className="text-[11px] f-body" style={{ color: T.muted }}>{hint}</p>}
    </div>
  );
}

function OverviewTab({ textMain, border, videoList, productList, calendarList }) {
  const totalSales = productList.reduce((a, p) => a + (Number(p.sales) || 0), 0);
  const totalComm = productList.reduce((a, p) => a + (Number(p.comm) || 0), 0);
  const totalOrders = productList.reduce((a, p) => a + (Number(p.orders) || 0), 0);
  const totalClicks = videoList.reduce((a, v) => a + (Number(v.clicks) || 0), 0);
  const totalViews = videoList.reduce((a, v) => a + parseViews(v.views), 0);
  const cvr = totalClicks > 0 ? ((totalOrders / totalClicks) * 100).toFixed(1) + "%" : "0%";
  const aov = totalOrders > 0 ? Math.round(totalSales / totalOrders) : 0;

  const salesByVideo = videoList.map(v => ({ name: v.title.length > 12 ? v.title.slice(0, 12) + "…" : v.title, sales: Number(v.sales) || 0 }));
  const commByVideo = videoList.map(v => ({ name: v.title.length > 10 ? v.title.slice(0, 10) + "…" : v.title, comm: Number(v.comm) || 0 }));
  const salesByProduct = productList.map(p => ({ name: p.name.length > 10 ? p.name.slice(0, 10) + "…" : p.name, sales: Number(p.sales) || 0 }));

  const funnelReal = [
    { stage: "ยอดวิว", value: totalViews, icon: Eye },
    { stage: "คลิก", value: totalClicks, icon: MousePointerClick },
    { stage: "ออเดอร์", value: totalOrders, icon: ShoppingBag },
  ];
  const funnelMax = funnelReal[0].value || 1;
    ...
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
      <div className="lg:col-span-3 flex flex-col gap-4">
        <div>
          ...
          <p className="text-sm" style={{ color: T.muted }}>ภาพรวมนี้คำนวณจากข้อมูลจริงที่คุณกรอกไว้ในหน้าคลิปวิดีโอและสินค้า</p>
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <KpiCard icon={DollarSign} label="ยอดขายรวม" value={"฿" + totalSales.toLocaleString()} accent={T.red} />
          <KpiCard icon={Wallet} label="คอมมิชชั่นรวม" value={"฿" + totalComm.toLocaleString()} accent={T.violet} />
          <KpiCard icon={ShoppingBag} label="ออเดอร์รวม" value={totalOrders.toLocaleString()} accent={T.cyan} />
          <KpiCard icon={MousePointerClick} label="คลิกทั้งหมด" value={totalClicks.toLocaleString()} accent={T.orange} />
          <KpiCard icon={Percent} label="Conversion Rate" value={cvr} accent={T.red} />
          <KpiCard icon={TrendingUp} label="มูลค่าออเดอร์เฉลี่ย" value={"฿" + aov.toLocaleString()} accent={T.violet} />
          <KpiCard icon={Eye} label="ยอดวิวรวม" value={formatCompact(totalViews)} accent={T.cyan} />
          <KpiCard icon={Video} label="จำนวนคลิปทั้งหมด" value={videoList.length.toLocaleString()} accent={T.orange} />
        </div>

        <SectionCard title="ยอดขายตามคลิป" subtitle="คำนวณจากคลิปที่คุณเพิ่มไว้ในหน้าคลิปวิดีโอ">
          {hasVideos ? (
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={salesByVideo}>
                <defs>
            ...
            <EmptyState icon={Video} text="ยังไม่มีข้อมูลคลิป" hint="ไปที่หน้า “คลิปวิดีโอ” แล้วเพิ่มคลิปแรกของคุณ" />
          )}
        </SectionCard>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <SectionCard title="คอมมิชชั่นตามคลิป">
            {hasVideos ? (
              <ResponsiveContainer width="100%" height={180}>
                <LineChart data={commByVideo}>
                  <CartesianGrid strokeDasharray="3 3" stroke={border} vertical={false} />
                  ...
                  <Tooltip contentStyle={{ borderRadius: 12, border: `1px solid ${border}`, fontSize: 12 }} />
                  <Line type="monotone" dataKey="comm" stroke={T.violet} strokeWidth={2} dot={{ r: 3 }} />
                </LineChart>
              </ResponsiveContainer>
            ) : (
              <EmptyState icon={Wallet} text="ยังไม่มีข้อมูล" />
            )}
          </SectionCard>
          <SectionCard title="ยอดขายตามสินค้า">
            {hasProducts ? (
              <ResponsiveContainer width="100%" height={180}>
                <BarChart data={salesByProduct}>
                  ...
                  <YAxis tick={{ fontSize: 10, fill: T.muted }} axisLine={false} tickLine={false} />
                  <Tooltip contentStyle={{ borderRadius: 12, border: `1px solid ${border}`, fontSize: 12 }} />
                  <Bar dataKey="sales" fill={T.cyan} radius={[6, 6, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            ) : (
              <EmptyState icon={Package} text="ยังไม่มีข้อมูลสินค้า" hint="ไปที่หน้า “สินค้า” แล้วเพิ่มสินค้าแรกของคุณ" />
            )}
          </SectionCard>
        </div>

        <SectionCard title="Sales Funnel" subtitle="จากยอดวิวสู่ออเดอร์ · คำนวณจากข้อมูลจริง">
          {hasVideos || hasProducts ? (
            <div className="flex flex-col gap-2">
              {funnelReal.map((f, i) => {
                const pct = funnelMax > 0 ? (f.value / funnelMax) * 100 : 0;
                const Icon = f.icon;
                return (
                  <div key={f.stage} className="flex items-center gap-3">
                    <div className="w-20 flex items-center gap-1.5 text-xs f-body" style={{ color: T.muted }}>
                    ...
                    <div className="flex-1 h-8 rounded-lg overflow-hidden" style={{ background: T.paper }}>
                      <div
                        ...
                        style={{ width: Math.max(pct, 6) + "%", background: [T.red, T.violet, T.cyan][i], minWidth: 46 }}
                      >
            ...
            <EmptyState icon={TrendingUp} text="ยังไม่มีข้อมูลพอสำหรับสรุป" />
          )}
        </SectionCard>
      </div>
      ...
      <div className="flex flex-col gap-4">
        <BotGuideCard calendarList={calendarList} />
        <SectionCard title="สินค้าขายดี">
          {hasProducts ? (
            <div className="flex flex-col gap-3">
              {[...productList].sort((a, b) => b.comm - a.comm).slice(0, 3).map(p => (
                <div key={p.id} className="flex items-center gap-3">
                  ...
                  <span className="text-xs f-mono font-medium" style={{ color: T.ink }}>฿{Number(p.comm).toLocaleString()}</span>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-xs f-body" style={{ color: T.muted }}>ยังไม่มีสินค้า — เพิ่มได้ที่หน้า “สินค้า”</p>
          )}
        </SectionCard>
        <SectionCard title="คลิปที่มาแรง">
          {hasVideos ? (
            <div className="flex flex-col gap-3">
              {[...videoList].sort((a, b) => parseViews(b.views) - parseViews(a.views)).slice(0, 3).map(v => (
            ...
            <p className="text-xs f-body" style={{ color: T.muted }}>ยังไม่มีคลิป — เพิ่มได้ที่หน้า “คลิปวิดีโอ”</p>
          )}
        </SectionCard>
        <SectionCard title="เริ่มต้นใช้งาน">
          <div className="flex flex-col gap-3 text-xs f-body">
            <NotifRow icon={Video} color={T.red} text="เพิ่มคลิปแรกของคุณที่หน้า “คลิปวิดีโอ”" time="ยังไม่ได้ทำ" />
            <NotifRow icon={Package} color={T.orange} text="เพิ่มสินค้าที่ปักตะกร้าไว้ที่หน้า “สินค้า”" time="ยังไม่ได้ทำ" />
            <NotifRow icon={CalendarIcon} color={T.violet} text="วางแผนวันถ่าย/โพสต์ที่หน้า “ปฏิทินงาน”" time="ยังไม่ได้ทำ" />
          </div>
        </SectionCard>
      </div>
    </div>
...
function BotGuideCard({ calendarList = [] }) {
  const shootDays = calendarList.filter(c => c.tag === "shoot").length;
  const editDays = calendarList.filter(c => c.tag === "edit").length;
  const postDays = calendarList.filter(c => c.tag === "post").length;
  const hasPlan = calendarList.length > 0;

  const tasks = [
    { label: "วันถ่ายคลิปที่วางแผนไว้", value: `${shootDays} วัน` },
    { label: "วันตัดต่อที่วางแผนไว้", value: `${editDays} วัน` },
    { label: "วันโพสต์คิวที่วางแผนไว้", value: `${postDays} วัน` },
  ];
  return (
    <div className="glow-edge is-active rounded-2xl p-5 text-white shadow-sm" style={{ background: T.ink }}>
      ...
      {hasPlan ? (
          ...
          {tasks.map(t => (
            <div key={t.label} className="flex items-center justify-between text-xs f-body">
              <div className="flex items-center gap-2">
                <Circle size={14} color="#5A5A66" />
                <span style={{ color: "white" }}>{t.label}</span>
              </div>
        ...
        <p className="text-xs f-body" style={{ color: "#B4B4C0" }}>
          ยังไม่มีแผนงาน — ไปที่หน้า “ปฏิทินงาน” แล้วเพิ่มวันถ่าย/ตัดต่อ/โพสต์ของคุณ
        </p>
      )}
    </div>
  );
...
function CalendarTab({ calendarList, setCalendarList }) {
  ...
  const addEntry = async () => {
    ...
    if (supabase) {
      await supabase.from("calendar").delete().eq("date", d);
      const { data, error } = await supabase.from("calendar").insert({ date: d, tag: newTag }).select();
      if (error) console.error(error.message);
      setCalendarList(prev => [...prev.filter(c => c.date !== d), (data && data[0]) || { date: d, tag: newTag }]);
    } else {
  ...
  const removeEntry = async (date) => {
    if (supabase) {
      await supabase.from("calendar").delete().eq("date", date);
    }
...
function VideosTab({ videoList, setVideoList, border }) {
  ...
  const addVideo = async () => {
    ...
    if (supabase) {
      ...
      setVideoList(prev => [data[0], ...prev]);
    } else {
      setVideoList(prev => [{ id: nextId(), ...row }, ...prev]);
    }
    setForm(empty);
  };
...
function ProductsTab({ border, platform, setPlatform, filteredProducts, setProductList }) {
  ...
  const addProduct = async () => {
    ...
    if (supabase) {
      ...
      if (error) { console.error(error.message); return; }
      setProductList(prev => [data[0], ...prev]);
    } else {
      setProductList(prev => [{ id: nextId(), ...row }, ...prev]);
    }
    setForm(empty);
  };
...
function SponsorsTab({ border, sponsorList, setSponsorList }) {
  ...
  const addSponsor = async () => {
    ...
    const row = { brand: form.brand, type: form.type || "-", value: Number(form.value) || 0, status: form.status, due: form.due || "-" };
    if (supabase) {
      const { data, error } = await supabase.from("sponsors").insert(row).select();
      if (error) { console.error(error.message); return; }
      setSponsorList(prev => [data[0], ...prev]);
    } else {
      setSponsorList(prev => [{ id: nextId(), ...row }, ...prev]);
    }
    setForm(empty);
  };

  const removeSponsor = async (id) => {
    if (supabase) await supabase.from("sponsors").delete().eq("id", id);
...
/* ---------- Live ---------- */
function LiveTab({ liveStats, setLiveStats }) {
  const [editing, setEditing] = useState(false);
  const [form, setForm] = useState(liveStats);

  const openEdit = () => { setForm(liveStats); setEditing(true); };

  const save = async () => {
    const row = {
      sessions: Number(form.sessions) || 0,
      sales: Number(form.sales) || 0,
      avg_viewers: Number(form.avg_viewers) || 0,
      peak_viewers: Number(form.peak_viewers) || 0,
    };
    if (supabase) {
      const { data, error } = await supabase.from("live_stats").upsert({ id: 1, ...row }).select();
      if (error) { console.error(error.message); return; }
      setLiveStats(data[0]);
    } else {
      setLiveStats({ id: 1, ...row });
    }
    setEditing(false);
  };
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <p className="text-xs f-body" style={{ color: T.muted }}>ตัวเลขไลฟ์ยังไม่ได้ดึงอัตโนมัติจาก TikTok — กรอกเองได้ตรงนี้</p>
        {!editing ? (
          <button onClick={openEdit} className="text-xs f-body font-medium px-3 py-1.5 rounded-lg border" style={{ borderColor: T.line, color: T.ink }}>
            แก้ไขตัวเลข
          </button>
        ) : (
          <div className="flex items-center gap-2">
            <button onClick={() => setEditing(false)} className="text-xs f-body px-3 py-1.5 rounded-lg border" style={{ borderColor: T.line, color: T.muted }}>ยกเลิก</button>
            <button onClick={save} className="text-xs f-body font-medium text-white px-3 py-1.5 rounded-lg" style={{ background: T.ink }}>บันทึก</button>
          </div>
        )}
      </div>
      ...
      {editing ? (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <SectionCard title="ไลฟ์ทั้งหมด (ครั้ง)">
            <TinyInput type="number" value={form.sessions} onChange={e => setForm({ ...form, sessions: e.target.value })} className="w-full" />
          </SectionCard>
          <SectionCard title="ยอดขายจากไลฟ์ (฿)">
            <TinyInput type="number" value={form.sales} onChange={e => setForm({ ...form, sales: e.target.value })} className="w-full" />
          </SectionCard>
          <SectionCard title="ผู้ชมเฉลี่ย">
            <TinyInput type="number" value={form.avg_viewers} onChange={e => setForm({ ...form, avg_viewers: e.target.value })} className="w-full" />
          </SectionCard>
          <SectionCard title="ผู้ชมสูงสุด">
            <TinyInput type="number" value={form.peak_viewers} onChange={e => setForm({ ...form, peak_viewers: e.target.value })} className="w-full" />
          </SectionCard>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="glow-edge is-active">
            <KpiCard icon={Radio} label="ไลฟ์ทั้งหมด" value={`${liveStats.sessions.toLocaleString()} ครั้ง`} accent={T.red} />
          </div>
          <KpiCard icon={DollarSign} label="ยอดขายจากไลฟ์" value={`฿${liveStats.sales.toLocaleString()}`} accent={T.violet} />
          <KpiCard icon={Users} label="ผู้ชมเฉลี่ย" value={liveStats.avg_viewers.toLocaleString()} accent={T.cyan} />
          <KpiCard icon={TrendingUp} label="ผู้ชมสูงสุด" value={liveStats.peak_viewers.toLocaleString()} accent={T.orange} />
        </div>
      )}
    </div>
  );
